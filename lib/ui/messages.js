 ...
 * @param {string} aiTool - Ferramenta de IA selecionada (claude, antigravity, codex)
 */
export function printSuccess(projectName, aiTool = 'claude') {
  ...
  } else if (aiTool === 'antigravity') {
    console.log(`    ${chalk.cyan('antigravity')}                   ${chalk.dim('# Start Antigravity')}`);
  } else if (aiTool === 'codex') {
    console.log(`    ${chalk.cyan('codex')}                         ${chalk.dim('# Start Codex')}`);
  }
  console.log('');
