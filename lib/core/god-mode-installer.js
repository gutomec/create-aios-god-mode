...
export async function installGodMode(projectDir, options = {}) {
  ...
  try {
    ...
    let skillsDest = path.join(claudeDir, 'skills', 'aios-god-mode');

    if (await fse.pathExists(skillsSrc)) {
      if (aiTool === 'antigravity') {
        skillsDest = path.join(projectDir, '.agent', 'workflows', 'aios-god-mode');
        await convertToAntigravity(skillsSrc, skillsDest);
      } else if (aiTool === 'codex') {
        skillsDest = path.join(projectDir, '.codex', 'skills', 'aios-god-mode');
        await convertToCodex(skillsSrc, skillsDest);
      } else {
