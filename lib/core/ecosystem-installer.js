 ...
 */
export async function configureMcps(projectDir, aiTool = 'claude') {
  if (aiTool === 'claude') {
  ...
  const result = await convertMcpConfig(MCP_CONFIG, projectDir, aiTool);
  return {
...
export async function cleanupSkillsStructure(projectDir, aiTool = 'claude') {
  ...
  if (aiTool !== 'claude' && await fse.pathExists(claudeSkillsDir)) {
    await convertSkills(claudeSkillsDir, projectDir, aiTool);
  }

  // 3. Converter rules
  const claudeRulesDir = path.join(projectDir, '.claude', 'rules');
  if (aiTool !== 'claude' && await fse.pathExists(claudeRulesDir)) {
    await convertRules(claudeRulesDir, projectDir, aiTool);
  }

  // 4. Converter project instructions (CLAUDE.md → GEMINI.md / AGENTS.md)
  await convertProjectInstructions(projectDir, aiTool);

  // 5. Remover .agents/ da raiz (criado pelo npx skills add)
  if (await fse.pathExists(agentsDir)) {
    // Para ferramentas que usam .agents/ (antigravity, codex), manter se necessário
    const toolPaths = getToolPaths(aiTool);
    if (!toolPaths.skills.startsWith('.agents')) {
