...
export async function initCommand(projectName, options = {}) {
  try {
    ...
    // ── Step 7: Compatibility Setup (Antigravity/Codex) ──
    if (aiTool === 'antigravity' || aiTool === 'codex') {
      const toolName = aiTool === 'antigravity' ? 'Antigravity' : 'Codex';
      printStep(7, TOTAL_STEPS, `Configuring ${toolName} compatibility...`);
      ...
      printStep(7, TOTAL_STEPS, 'Skipping compatibility setup (Claude Code selected)');
      console.log(chalk.dim('  Native Claude Code support active.'));
    }

    // ── Step 8: Install GSD + cc-deploy ──
