...
import { setupAntigravity, setupCodex } from '../core/compatibility-installer.js';
...
export async function initCommand(projectName, options = {}) {
  try {
    ...
    const { name, projectType, language, squads, aiTool } = config;
    ...
    printStep(2, TOTAL_STEPS, 'Initializing AIOS framework...');

    let spinner = ora('Initializing AIOS framework...').start();

    try {
      const resultDir = await bootstrapAios(name, process.cwd());
      spinner.succeed('AIOS framework initialized');
    } catch (error) {
      spinner.fail('Failed to initialize AIOS framework');
    ...
    try {
      const { version, filesInstalled } = await installGodMode(projectDir);
    ...
    // ── Step 6: Compatibility Setup (Antigravity/Codex) ──
    if (aiTool === 'antigravity' || aiTool === 'codex') {
      const toolName = aiTool === 'antigravity' ? 'Antigravity' : 'Codex';
      printStep(4, TOTAL_STEPS, `Configuring ${toolName} compatibility...`);
      spinner = ora(`Setting up ${toolName}...`).start();

      try {
        if (aiTool === 'antigravity') {
          await setupAntigravity(projectDir);
        } else {
          await setupCodex(projectDir);
        }
        spinner.succeed(`${toolName} configured successfully`);
      } catch (error) {
        spinner.fail(`Failed to configure ${toolName}`);
        printWarning(`${toolName} setup failed: ${error.message}`);
      }
    } else {
      printStep(4, TOTAL_STEPS, 'Skipping compatibility setup (Claude Code selected)');
      console.log(chalk.dim('  Native Claude Code support active.'));
    }

    // ── Step 7: Install Squads ──
    printStep(5, TOTAL_STEPS, 'Installing squads...');
    ...
    printStep(6, TOTAL_STEPS, 'Configuring MCP servers...');

    spinner = ora('Configuring MCP servers...').start();

    try {
      const mcpCount = await configureMcps(projectDir);
      spinner.succeed(`${mcpCount} MCP servers configured ${chalk.dim('(nano-banana-pro, context7, 21st-dev)')}`);
    } catch (error) {
      spinner.fail('Failed to configure MCP servers');
      printWarning(`MCP configuration failed: ${error.message}. You can configure manually in .mcp.json`);
    }

    // ── Step 9: Install Ecosystem Skills ──
    printStep(7, TOTAL_STEPS, `Installing ecosystem skills (${getSkillCount()})...`);
    ...
    try {
      await cleanupSkillsStructure(projectDir);
      console.log(chalk.green('  ✓') + ' Cleaned up directory structure');
    } catch (error) {
      printWarning(`Directory cleanup failed: ${error.message}`);
    }

    // ── Step 10: Install GSD + cc-deploy ──
    printStep(8, TOTAL_STEPS, 'Installing GSD framework...');
    ...
    // ── Step 11: Post-Setup ──
    printStep(9, TOTAL_STEPS, 'Installing dependencies...');
    ...
    try {
      ...
      printStep(10, TOTAL_STEPS, 'Initializing git repository...');
